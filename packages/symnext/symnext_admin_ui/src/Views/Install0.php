...
class Install extends View
{
    ...
    protected function beforeGenerate()
    {
        ...
        $config->read(\Symnext\TEMPLATES . '/config-default.xml');
        $fields = $_POST['fields'];
        array_walk_recursive($fields, 'trim');

        // Check for errors.
        $errors = [];
        $tree = $this->tree;

        $value = $fields['general']['site_name'];
        $config->set('site_name', $value, 'general');
        if ($value == '') {
            $this->addError('fields[general][site_name]', 'required');
        }
        $value = $fields['admin']['admin_path'];
        $config->set('admin_path', $value, 'admin');
        if ($value == '') {
            $this->addError('fields[admin][admin_path]', 'required');
        }
        $value = $fields['database']['db'];
        $config->set('database', $value, 'database');
        if ($value == '') {
            $this->addError('fields[database][db]', 'required');
        }
        $value = $fields['database']['user'];
        $config->set('user', $value, 'database');
        if ($value == '') {
            $this->addError('fields[database][user]', 'required');
        }
        $value = $fields['database']['password'];
        $config->set('password', $value, 'database');
        if ($value == '') {
            $this->addError('fields[database][password]', 'required');
        }

        $config->write(sys_get_temp_dir() . '/config.xml');
        $tree->params->config_file = sys_get_temp_dir() . '/config.xml';
    }

    protected function buildView(): void
    {
        $tree = $this->tree;
        #$tree->params->addChild('php-version', PHP_VERSION);
        #$tree->params->addChild('php-version-min', '7.3.0');
        $tree->params->php_version = PHP_VERSION;
        $tree->params->php_version_min = '7.3.0';
        if (!isset($tree->params->config_file)) {
            $tree->params->config_file = \Symnext\TEMPLATES . '/config-default.xml';
        }
        $requirements_not_met = self::checkRequirements();
        if (!empty($requirements_not_met)) {
            $x_req = $tree->addChild('requirements');
            foreach ($requirements_not_met as $error) {
                $x_req->addChild('requirement', $error);
            }
        } else {
            $dtz_array = DateTimeObj::getTimezonesSelectOptions();
            #print_r($dtz_array); die;
            $data = $tree->addChild('form');
            $zones = $this->tree->addChild('timezones');
            foreach ($dtz_array as $region) {
                $el_region = $zones->addChild('optgroup');
                $el_region->addAttribute('label', $region['label']);
                foreach ($region['options'] as $option) {
                    $el_option = $el_region->addChild('option', $option[2]);
    ...
    {
        if (!isset($this->tree->errors)) {
            $this->tree->addChild('errors');
        }
        $x_errors = $this->tree->errors;
        $x_error = $x_errors->addChild('error', $field);
