...
class SMTP
{
     ...
     */
    protected function _send(string $request): bool|int
    {
     ...
     * @return string
     */
    protected function _receive(int $timeout = null): string
    {
        $this->checkConnection();
        ...
        if (!empty($info['timed_out'])) {
            throw new SMTPException(__('%s has timed out', [$this->_host]));
        } elseif ($response === false) {
            throw new SMTPException(__('Could not read from %s', [$this->_host]));
        }
     ...
     */
    protected function _expect(string|array $code, int $timeout = null): string
    {
        $cmd  = '';
        $more = '';
        $msg  = '';
     ...
     * @return void
     */
    protected function _connect(string $host, int $port, array $options = []): void
    {
        $errorNum = 0;
        $errorStr = '';
