...
class Configuration
{
    ...
    {
        //$this->tree = new XMLElement('<configuration></configuration>');
        $this->_forceLowerCase = $forceLowerCase;
    }

    /**
     * Accessor function for the `$this->tree`.
     *
     * @param string $path
    ...
    {
        return (string)$this->tree->xpath($path);
    }
    ...
    {
        $file = $file ?? CONFIG;
        $this->tree = new XMLElement(file_get_contents($file));
    }
    ...
    public function write(string $file = null, int $permissions = null): bool
    {
        if (is_null($permissions) && $this->get('files/file_write_mode') !== null) {
            $permissions = $this->properties['files/file_write_mode'];
        }
        return General::writeFile($file, $this->tree->asXML(), $permissions);
    }
