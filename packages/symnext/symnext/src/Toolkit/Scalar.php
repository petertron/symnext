...
/**
 * Scalar is a utility class that performs operations on numbers and strings.
 */
class Scalar
{
    /**
    ...
    {
        return $source ? htmlspecialchars($source, ENT_COMPAT, 'UTF-8', false) : '';
    }
     ...
     */
    public static function sanitizeDouble(string $source): string
    {
        return htmlspecialchars($source, ENT_COMPAT, 'UTF-8', true);
    }
     ...
     */
    public static function reverse_sanitize(string $str): string
    {
     ...
     */
    public static function tabsToSpaces(string $string, int $spaces = 4)
    {
     ...
     *  instead.
     * @param XSLTProcess $xslt_processor (optional)
    ...
    public static function validateXML(
        ...
        array &$errors = null,
        bool $is_file = true,
        XSLTProcess $xslt_processor = null,
        string $encoding = 'UTF-8'
    ): bool
    {
        $_data = ($is_file) ? file_get_contents($data) : $data;
        ...
        if (is_object($xslt_processor)) {
            ...
            $xslt_processor->process($_data, $xsl);

            if ($xslt_processor->isErrors()) {
                $errors = $xslt_processor->getError(true);
                return false;
            }
        } else {
     ...
     */
    public static function validateURL(string $url = null): string
    {
        $url = trim($url);
     ...
     */
    protected static function flattenArraySub(
        array &$output, array &$source, string $path
    )
    {
        foreach ($source as $key => $value) {
            $key = $path . ':' . $key;
     ...
     */
    public static function createHandle(string $string, int $max_length = 255, $delim = '-', $uriencode = false, $additional_rule_set = null): string
    {
     ...
     */
    public static function createFilename(string $string, ?string $delim = '-'): string
    {
        // Strip out any tag
        $string = strip_tags($string);
     ...
     */
    public static function substrmin(string $str, int $val): string|bool
    {
     ...
     *  the resulting string or false on failure.
     */
    public static function substrmax(string $str, int $val): string|bool
    {
     ...
     */
    public static function right(string $str, int $num): string|bool
    {
     ...
     *  input string, or false on failure.
     */
    public static function left(string $str, int $num): string|bool
    {
     ...
     * @return boolean
     */
    public static function realiseDirectory(
        string $path,
        int|string $mode = 0755,
        bool $silent = true
    ): bool
    {
        if (is_dir($path)) {
        ...
        try {
            ...
            if ($silent === false) {
                throw new Exception(__('Unable to create path - %s', [$path]));
            }
     ...
     * @return boolean
     */
    public static function deleteDirectory(string $dir, bool $silent = true)
    {
        try {
            if (!@file_exists($dir)) {
                return true;
            ...
            if ($silent === false) {
                throw new Exception(__('Unable to remove - %s', [$dir]));
            }
    ...
    public static function array_to_xml(XMLElement $parent, array $data, $validate = false)
    {
        foreach ($data as $element_name => $value) {
            ...
            if (is_int($element_name)) {
                ...
                $child = new XMLElement($element_name, null, [], true);
            }
     ...
     */
    public static function deleteFile(string $file, $silent = true)
    {
        try {
     ...
     */
    public static function listDirStructure($dir = '.', $filter = null, $recurse = true, $strip_root = null, $exclude = [], $ignore_hidden = true)
    {
        if (!is_dir($dir)) {
            return null;
        }
     ...
     * the input directory is not actually a directory.
     */
    public static function listStructure($dir = ".", $filters = [], $recurse = true, $sort = "asc", $strip_root = null, $exclude = [], $ignore_hidden = true)
    {
        if (!is_dir($dir)) {
            return null;
        }
     ...
     */
    public static function uploadFile(string $dest_path, string $dest_name, string $tmp_name, $perm = 0644)
    {
        // Upload the file
    ...
    public static function formatFilesize(int $file_size): string
    {
        ...
        if ($file_size >= (1024 ^ 3)) {
            $file_size = number_format($file_size / (1024 ^ 3), 2) . ' GiB';
        } elseif ($file_size >= (1024 ^ 2)) {
            $file_size = number_format($file_size / (1024 ^ 2), 2) . ' MiB';
        } elseif ($file_size >= 1024) {
            $file_size = intval($file_size / 1024) . ' KiB';
        } else {
     ...
     *  The wrapped string
     */
    public static function wrapInCDATA(string $value = null)
    {
        if (empty($value)) {
            return $value;
        }
