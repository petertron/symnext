...
class PageManager
{
     ...
     *  The Page title
     */
    public static function fetchTitleFromHandle(string $handle): string
    {
        return App::Database()
            ->select(['title'])
            ->from('tbl_pages')
     ...
     *  The Page ID
     */
    public static function fetchIDFromHandle(string $handle): int
    {
        return (int)App::Database()
            ->select(['id'])
            ->from('tbl_pages')
            ->where(['handle' => $handle])
            ->limit(1)
    ...
    {
        return General::writeFile($path, $data, App::Configuration()->get('write_mode', 'file'));
    }
     ...
     * @return boolean
     */
    public static function editPageChildren(
        int $page_id = null,
    ...
    ): bool
    {
        ...
        $page = PageManager::fetchPageByID($page_id, ['path', 'handle']);
        ...
        if ($can_proceed) {
            ...
            App::Database()
                ...
                ->where(['sortorder' => ['>' => $page['sortorder'] ?? null]])
                ->execute();
        }
     ...
     * @return boolean
     */
    public static function deletePageTypes(int $page_id = null): bool
    {
        if (!$page_id) {
            return false;
        }
    ...
    {
        if (App::Log()) {
            App::Log()->pushDeprecateWarningToLog('PageManager::fetch()', 'PageManager::select()');
        }
     ...
     *  null is returned.
     */
    public static function fetchPageByID(
        int|array $page_id = null,
        array $select = []
    ): array|null
    {
        if (!$page_id) {
     ...
     *  null is returned.
     */
    public static function fetchChildPages(
        int $page_id = null,
        array $select = []
    ): array|null
    {
