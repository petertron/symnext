...
class CacheDatabase implements iNamespacedCache
{
    ...
    /**
     * Given the hash of some data, check to see whether it exists in
     * `tbl_cache`.
     * If no cached object is found, this function will return false, otherwise
     * the cached object will be returned as an array.
     *
     * @param string $hash
     ...
     *  expiry time, the hash and the data. If the object is not found, false
     *  will be returned.
     */
    public function read(
        string $hash = null, string $namespace = null
    ): array|bool
    {
        if (!$hash && !$namespace) {
            return false;
     ...
     */
    private function __optimise(): void
    {
