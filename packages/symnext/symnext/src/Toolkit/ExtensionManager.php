...
class ExtensionManager #implements FileResource
{
    ...
    {
        if (empty(self::$_subscriptions) && App::Database() && App::Database()->isConnected()) {
     ...
     * @return string
     */
    public static function __getClassPath(string $name): string
    {
     ...
     * @return string
     */
    public static function __getDriverPath(string $name): string
    {
     ...
     * @return Extension
     */
    public static function getInstance(string $name): Extension
    {
     ...
     */
    protected static function buildExtensionList(bool $update = false): void
    {
        self::$extensions = array_unique(
            InstalledVersions::getInstalledPackagesByType('symnext-extension')
        );
    }

    public static function getExtensionInfo()
    {
        self::buildExtensionList();
        $return = [];
        if (!empty(self::$extensions)) {
            foreach (self::$extensions as $path) {
                $file = VENDOR . '/' . $path . '/composer.json';
                if (is_file($file)) {
     ...
     */
    public static function fetchStatus(array $about): array
    {
        $return = [];
     ...
     * @return string
     */
    public static function fetchInstalledVersion(string $name): string
    {
        static::buildExtensionList();
     ...
     * @return integer
     */
    public static function fetchExtensionID(string $name): int
    {
        static::buildExtensionList();

        return self::$_extensions[$name]['id'] ?? null;
    }
     ...
     *  An array of objects
     */
    public static function getProvidersOf(string $type = null): array
    {
     ...
     * @return Cacheable
     */
    public static function getCacheProvider(string $key, bool $reuse = true)
    {
        $cacheDriver = App::Configuration()->get($key, 'caching');

        if (in_array($cacheDriver, array_keys(App::ExtensionManager()->getProvidersOf('cache')))) {
            $cacheable = new $cacheDriver;
        } else {
            $cacheable = App::Database();
     ...
     * @return boolean
     */
    protected static function requiresInstallation(string $name): bool
    {
        static::buildExtensionList();
        $id = self::$_extensions[$name]['id'] ?? null;
     ...
     */
    protected static function requiresUpdate(string $name, string $file_version): bool
    {
     ...
     *  The Extension ID
     */
    public static function registerDelegates(string $name): int|bool
    {
        $obj = self::getInstance($name);
        $id = self::fetchExtensionID($name);
     ...
     * @return boolean
     */
    public static function removeDelegates(string $name): bool
    {
        $delegates = App::Database()
            ->select(['ted.id'])
    ...
    protected static function canUninstallOrDisable(Extension $obj)
    {
        ...
        if (is_dir(EXTENSIONS . "/{$extension_handle}/fields")) {
            foreach (glob(EXTENSIONS . "/{$extension_handle}/fields/field.*.php") as $file) {
                ...
                if (FieldManager::isFieldUsed($type)) {
                    throw new Exception(
                        __('The field ‘%s’, provided by the Extension ‘%s’, is currently in use.', [basename($file), $about['name']])
        ...
        if (is_dir(EXTENSIONS . "/{$extension_handle}/data-sources")) {
            foreach (glob(EXTENSIONS . "/{$extension_handle}/data-sources/data.*.php") as $file) {
                ...
                if (PageManager::isDataSourceUsed($handle)) {
                    throw new Exception(
                        __('The Data Source ‘%s’, provided by the Extension ‘%s’, is currently in use.',[basename($file), $about['name']])
        ...
        if (is_dir(EXTENSIONS . "/{$extension_handle}/events")) {
            foreach (glob(EXTENSIONS . "/{$extension_handle}/events/event.*.php") as $file) {
                ...
                if (PageManager::isEventUsed($handle)) {
                    throw new Exception(
                        __('The Event ‘%s’, provided by the Extension ‘%s’, is currently in use.', [basename($file), $about['name']])
                        . ' ' . __("Please remove it from your pages prior to uninstalling or disabling.")
                    );
                }
            }
        ...
        if (is_dir(EXTENSIONS . "/{$extension_handle}/text-formatters")) {
            foreach (glob(EXTENSIONS . "/{$extension_handle}/text-formatters/formatter.*.php") as $file) {
                $handle = preg_replace(['/^formatter\./i', '/\.php$/i'], null, basename($file));

                if (FieldManager::isTextFormatterUsed($handle)) {
                    throw new Exception(
                        __('The Text Formatter ‘%s’, provided by the Extension ‘%s’, is currently in use.', [basename($file), $about['name']])
    ...
    )
    {
        ...
        $context += ['page' => $page, 'delegate' => $delegate];
        $profiling = App::Profiler() instanceof Profiler;
        ...
        foreach ($services as $s) {
            ...
            if ($profiling) {
                ...
                App::Profiler()->sample($delegate . '|' . $s['name'], PROFILE_LAP, 'Delegate', $queries);
            }
        }
    }
     ...
     * @return array
     */
    public static function listInstalledHandles(): array
    {
        if (empty(self::$_enabled_extensions) && App::Database()->isConnected()) {
     ...
     * @return boolean
     */
    public static function isInstalled(string $handle): bool
    {
     ...
     */
    public static function listAll(string $filter = '/^((?![-^?%:*|"<>]).)*$/'): array
    {
        $result = [];
     ...
     * @return integer
     */
    private static function sortByAuthor(array $a, array $b, int $i = 0): int
    {
        $first = $a;
        $second = $b;
    ...
    ): array
    {
        ...
        if (!is_null($order_by)) {
            ...
            if ($sort == 'author') {
                ...
                uasort($author, ['self', 'sortByAuthor']);
     ...
     */
    public static function about(string $name, bool $rawXML = false): array
    {
        ...
        if (file_exists(self::__getClassPath($name) . '/extension.meta.xml')) {
            try {
                ...
                App::Engine()->throwCustomError(
                    __('The %1$s file for the %2$s extension is not valid XML: %3$s', [
            ...
            if ($release = $xpath->query("//ext:release[@version='$latest_release_version']", $extension)->item(0)) {
                ...
                $symphonyVc = new VersionComparator(App::Configuration()->get('version', 'symphony'));
            ...
            App::Log()->pushToLog(sprintf('%s does not have an extension.meta.xml file', $name), E_DEPRECATED, true);
     ...
     * @return Extension
     */
    public static function create(string $name): Extension
    {
        if (!isset(self::$_pool[$name])) {
            ...
            if (!is_file($path)) {
                $errMsg = __('Could not find extension %s at location %s.', [
                    '<code>' . $name . '</code>',
    ...
    public static function cleanupDatabase(): void
    {
        ...
        if (!empty($rows)) {
            foreach ($rows as $r) {
                ...
                $removeDelegatesSQL = App::Database()
                    ->delete('tbl_extensions_delegates')
    ...
    {
        return new ExtensionQuery(App::Database(), $projection);
    }
}
