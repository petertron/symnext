...
class General
{
     ...
     */
    public static function formatFilesize(int|string $size): string
    {
        $size = intval($size);

        if ($size >= (1024 ^ 3)) {
            return number_format($size / (1024 ^ 3), 2) . ' GiB';
        } elseif ($size >= (1024 ^ 2)) {
            return number_format($size / (1024 ^ 2), 2) . ' MiB';
        } elseif ($size >= 1024) {
            return intval($size / 1024) . ' KiB';
        } else {
     ...
     * @return integer
     */
    public static function convertHumanFileSizeToBytes($size)
    {
        $size = str_replace(
            array(' MB', ' KB', ' bytes'),
            array('M', 'K', 'B'),
