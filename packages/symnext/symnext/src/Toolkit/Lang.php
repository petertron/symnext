...
class Lang
{
    ...
    public static function set($code, $checkStatus = true)
    {
        ...
        if ($code !== 'en' && (self::isLanguageEnabled($code) || $checkStatus === false)) {
            ...
            if ($code !== 'en' && App::Log() instanceof Log) {
                App::Log()->pushToLog(
                    __(
    ...
    public static function isLanguageEnabled($code)
    {
        ...
        if (App::ExtensionManager()) {
            $enabled_extensions = App::ExtensionManager()->listInstalledHandles();
        }
    ...
    public static function translate(
        string $string, array $inserts = null, string $namespace = null
    )
    {
        if (!$namespace) {
            $namespace = App::getPageNamespace();
        }
    ...
    public static function standardizeDate($string)
    {
        ...
        if (self::isLocalized()) {
            ...
            $separator = App::Configuration()->get('datetime_separator', 'region');
            if ($separator !== ' ') {
     ...
     */
    public static function createHandle($string, $max_length = 255, $delim = '-', $uriencode = false, $apply_transliteration = false, $additional_rule_set = null)
    {
