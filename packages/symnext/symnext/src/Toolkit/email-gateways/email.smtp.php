...
class SMTPGateway extends EmailGateway
{
     ...
     * @return void
     */
    public function setPass(string $pass = null): void
    {
        $this->_pass = $pass;
    }
     ...
     * @return void
     */
    public function setAuth(bool $auth = false): void
    {
        $this->_auth = $auth;
    }
     ...
     * @return void
     */
    public function setSecure(string $secure = null): void
    {
        if ($secure == 'tls') {
            $this->_protocol = 'tcp';
            $this->_secure = 'tls';
    ...
    public function getPreferencesPane()
    {
        ...
        $group->appendChild(new XMLElement('p', __('A fully qualified domain name (FQDN) of your server, e.g. "www.example.com". If left empty, Symphony will attempt to find an IP address for the EHLO/HELO greeting.'), ['class' => 'help']));
        ...
        $options = [
            ['no',$this->_secure == 'no', __('No encryption')],
            ['ssl',$this->_secure == 'ssl', __('SSL encryption')],
            ['tls',$this->_secure == 'tls', __('TLS encryption')],
        ];
        $select = Widget::Select('settings[email_smtp][secure]', $options, $readonly);
        $label->appendChild($select);
        $group->appendChild($label);

        $group->appendChild(new XMLElement('p', __('For a secure connection, SSL and TLS are supported. Please check the manual of your email provider for more details.'), ['class' => 'help']));
        ...
        $label->setValue(__('%s Requires authentication', [$input->generate()]));
        $group->appendChild($label);

        $group->appendChild(new XMLElement('p', __('Some SMTP connections require authentication. If that is the case, enter the username/password combination below.'), ['class' => 'help']));
        ...
        $label->setAttribute('class', 'column');
        $label->appendChild(Widget::Input('settings[email_smtp][username]', General::sanitize($this->_user), 'text', array_merge($readonly, ['autocomplete' => 'off'])));
        $div->appendChild($label);

        $label = Widget::Label(__('Password'));
        $label->setAttribute('class', 'column');
        $label->appendChild(Widget::Input('settings[email_smtp][password]', General::sanitize($this->_pass), 'password', array_merge($readonly, ['autocomplete' => 'off'])));
        $div->appendChild($label);
        $group->appendChild($div);
