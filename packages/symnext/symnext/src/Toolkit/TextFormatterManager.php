...
class TextformatterManager implements FileResource
{
    ...
    {
        return preg_replace(['/^formatter./i', '/.php$/i'], '', $filename);
    }
     ...
     */
    public static function create(string $handle): TextFormatter
    {
        if (!isset(self::$_pool[$handle])) {
            ...
            if (!is_file($path)) {
                throw new Exception(
                    __('Could not find Text Formatter %s.', ['<code>' . $classname . '</code>'])
