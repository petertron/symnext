...
abstract class SectionDatasource extends Datasource
{
     ...
     * @return boolean
     */
    public function canProcessSystemParameters(): bool
    {
    ...
    /**
     * Given a name for the group, and an associative array that contains
     * three keys, `attr`, `records` and `groups`. Grouping of Entries is
     * done by the grouping Field at a PHP level, not through the Database.
     *
     ...
     * @return XMLElement
     */
    public function processRecordGroup(
        string $element,
        array $group
    ): XMLElement
    {
        $xGroup = (new XMLElement($element, null))->setAttributeArray($group['attr']);
     ...
     */
    public function processEntry(Entry $entry): XMLElement|bool
    {
        $data = $entry->getData();
        ...
        if (in_array('system:date', $this->dsParamINCLUDEDELEMENTS)) {
            if (Symphony::Log()) {
                Symphony::Log()->pushDeprecateWarningToLog('system:date', 'system:creation-date` or `system:modification-date', [
     ...
     * @return void
     */
    private function appendFormattedError(
        string $handle,
        XMLElement &$xEntry,
        Throwable $ex
    ): void
        ...
        foreach ($this->dsParamPARAMOUTPUT as $param) {
            ...
            if ($param === 'system:id') {
                ...
                if ($param === 'system:date' && Symphony::Log()) {
                    Symphony::Log()->pushDeprecateWarningToLog('system:date', 'system:creation-date', [
     ...
     * @throws Exception
     */
    public function processFilters(EntryQuery &$entryQuery)
    {
        ...
        foreach ($this->dsParamFILTERS as $field_id => $filter) {
            ...
            if (!in_array($field_id, self::$_system_parameters) && $field_id != 'id' && !(self::$_fieldPool[$field_id] instanceof Field)) {
                throw new Exception(
                    __(
                        ...
                        [$field_id, '<code>' . $this->dsParamROOTELEMENT . '</code>']
                    )
                );
            }
            ...
            if ($field_id === 'system:id' || $field_id === 'id') {
                if ($field_id === 'id' && Symphony::Log()) {
                    Symphony::Log()->pushDeprecateWarningToLog('id', 'system:id', [
    ...
    public function execute(XMLElement &$wrapper, array &$param_pool = null): XMLElement
    {
        ...
        try {
            $result = $this->generate($wrapper, $param_pool);
     ...
     * @return XMLElement
     */
    public function generate(XMLElement &$wrapper, array &$param_pool = null)
    {
        ...
        $result = $wrapper->appendElement($this->dsParamROOTELEMENT);
        $sectioninfo = $wrapper->ownerDocument->createElement('section', General::sanitize($section->get('name')))->setAttributeArray([
            'id' => $section->get('id'),
        ...
        if ($this->_force_empty_result == true) {
            ...
            $error = new XMLElement('error', __("Data source not executed, required parameter is missing."), [
        ...
        if (isset($this->dsParamPARAMOUTPUT) && !is_array($this->dsParamPARAMOUTPUT)) {
            $this->dsParamPARAMOUTPUT = [$this->dsParamPARAMOUTPUT];
        }
         ...
         * @param array $filters
         */
        Symphony::ExtensionManager()->notifyMembers('DataSourceEntriesBuilt', '/frontend/', [
            'datasource' => &$this,
            'entries' => &$entries,
