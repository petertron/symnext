...
namespace Symnext\Toolkit\Events;

/**
 * The `SectionEvent` class provides methods required to save data entered on
 * the frontend to a corresponding Symphony section.
 *
...
class SectionEvent extends FilterableEvent
{
    ...
    ): XMLElement
    {
        ...
        $result->appendElement('message', __('Entry encountered errors when saving.'), ['message-id' => EventMessages::ENTRY_ERRORS]);
        ...
        foreach ($errors as $field_id => $message) {
            ...
            if (is_array($fields[$field->get('element_name')])) {
                $type = array_reduce($fields[$field->get('element_name')], ['SectionEvent', '__reduceType']);
            } else {
    ...
    {
        $error = new XMLElement($field->get('element_name'), null, [
     ...
     *  'missing' or 'invalid'
     */
    public static function __reduceType(array $a, array $b)
    {
        if (is_array($b)) {
            return array_reduce($b, ['SectionEvent', '__reduceType']);
        }
    ...
    public function execute()
    {
        ...
        if (in_array('expect-multiple', $this->eParamFILTERS)) {
            foreach ($post['fields'] as $position => $fields) {
                ...
                $entry = new XMLElement('entry', null, ['position' => $position]);
    ...
    ): XMLElement
    {
        ...
        if (!$section) {
            $result->setAttribute('result', 'error');
            $result->appendChild(new XMLElement('message', __('The Section, %s, could not be found.', [$this->getSource()]), [
        ...
        if (isset($entry_id)) {
            ...
            if (!$entry) {
                $result->setAttribute('result', 'error');
                $result->appendChild(new XMLElement('message', __('The Entry, %s, could not be found.', [$entry_id]), [
        ...
        if (Entry::__ENTRY_FIELD_ERROR__ == $entry->checkPostData($fields, $errors, ($entry->get('id') ? true : false))) {
            ...
            if (isset($entry_id)) {
                $result->appendChild(new XMLElement('message', __('Entry edited successfully.'), [
    ...
    ): XMLElement
    {
        ...
        $fields['subject'] = self::replaceFieldToken($send_email['subject'], $fields, __('[Symphony] A new entry was created on %s', [Symphony::Configuration()->get('sitename', 'general')]));
        ...
        if (!empty($errors)) {
            ...
            foreach ($errors as $type => $messages) {
                ...
                foreach ($messages as $recipient => $message) {
                    $xType->appendChild(
                        new XMLElement('message', General::wrapInCDATA($message), [
                            'recipient' => $recipient
                        ])
                    );
                }
