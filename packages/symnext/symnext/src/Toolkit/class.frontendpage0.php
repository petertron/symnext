...
class FrontendPage extends XSLTPage
{
     ...
     * @see __buildPage()
     * @param string $page
     * The URL of the current page that is being Rendered as returned by get_current_page
     * @throws Exception
    ...
    public function generate(string $page = null): string
    {
        ...
        if ($full_generate) {
             ...
             */
            Symphony::ExtensionManager()->notifyMembers(
                'FrontendPreRenderHeaders', '/frontend/'
            );
    ...
    private function __buildPage()
    {
        ...
        if (is_array($_GET) && !empty($_GET)) {
            foreach ($_GET as $key => $val) {
                if (in_array($key, ['symphony-page', 'debug', 'profile'])) {
                    continue;
                }
    ...
    public function resolvePage($page = null)
    {
         ...
         */
        Symphony::ExtensionManager()->notifyMembers('FrontendPrePageResolve', '/frontend/', ['row' => &$row, 'page' => &$this->_page]);
    ...
    private function processEvents($events, XMLElement &$wrapper)
    {
         ...
         */
        Symphony::ExtensionManager()->notifyMembers('FrontendProcessEvents', '/frontend/', [
            'env' => $this->_env,
            'events' => &$events,
            'wrapper' => &$wrapper,
         ...
         *  their name.
         */
        Symphony::ExtensionManager()->notifyMembers('FrontendEventPostProcess', '/frontend/', ['xml' => &$wrapper]);
    }
    ...
    public function processDatasources($datasources, XMLElement &$wrapper, array $params = [])
    {
        ...
        foreach ($dsOrder as $handle) {
             ...
             */
            Symphony::ExtensionManager()->notifyMembers('DataSourcePreExecute', '/frontend/', [
                'datasource' => &$ds,
                'xml' => &$xml,
            ...
            if ($xml) {
                 ...
                 *  The existing param pool including output parameters of any previous data sources
                 */
                Symphony::ExtensionManager()->notifyMembers('DataSourcePostExecute', '/frontend/', [
                    'datasource' => $ds,
                    'xml' => &$xml,
                    'param_pool' => &$this->_env['pool']
                ]);
