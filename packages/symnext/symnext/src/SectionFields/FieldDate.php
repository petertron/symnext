...
use Symnext\Database\EntryQueryDateAdapter;
...
class FieldDate extends Field implements ExportableField, ImportableField
{
     ...
     * @return array
     */
    public static function isEqualTo(
        array $parts, string $direction, bool $equal_to = false
    )
    {
        if (!$equal_to) {
            return $parts;
     ...
     * @return int
     */
    public static function parseFilter(string &$string)
    {
    ...
    {
        $x_parent->appendElement('pre_populate', $values['pre_populate'] ?? 'now');
    ...
    public function commit(): bool
    {
        ...
        $fields['pre_populate'] = $this->get('pre_populate') ?? '';
        $fields['time'] = $this->get('time') ?? 'no';
        $fields['calendar'] = $this->get('calendar') ?? 'no';
    ...
    ): int
    {
        ...
        if (!DateTimeObj::validate($data)) {
            $message = __('The date specified in ‘%s’ is invalid.', [$this->get('label')]);
    ...
    public function groupRecords(?array $records): ?array
    {
        ...
        foreach ($records as $r) {
            ...
            if (!isset($groups['year'][$year]['groups']['month'])) {
                $groups['year'][$year]['groups']['month'] = [];
            }

            if (!isset($groups['year'][$year]['groups']['month'][$month])) {
                $groups['year'][$year]['groups']['month'][$month] = [
                    'attr' => ['value' => $month],
                    'records' => [],
                    'groups' => []
                ];
