...
class DatabaseStatement
{
     ...
     */
    public function __construct(Database $db, string $statement = '')
    {
        $this->db = $db;
        if (!empty($statement)) {
     ...
     * @return array
     */
    final protected function getSQL(): array
    {
        return $this->sql;
    }
     ...
     * @return array
     */
    final public function getSQLParts(string|array $type): array
    {
     ...
     * @return bool
     */
    final public function containsSQLParts(string $type): bool
    {
     ...
     *  - `,` which joins part with a comma if both the preceding and next
     * parts are not empty
     *
     * @see getSQLParts()
     * @see generateSQL()
     ...
     */
    final public function generateSQL(): string
    {
     ...
     */
    final public function generateFormattedSQL(): string
    {
     ...
     */
    final public function generateOrderedSQLParts(): array
    {
     ...
     */
    final public function unsafeAppendSQLPart(
        string $type,
        string $part
    ): self
    {
        if (!General::in_array_multi($type, $this->getStatementStructure())) {
            ...
            throw new DatabaseStatementException(
                "SQL Part type `$type` is not valid for class `$class`"
            );
        }
        $this->sql[] = [$type => $part];
        return $this;
     ...
     *  The current instance
     */
    final protected function appendValues(array $values): self
    {
    ...
    /**
     * Statement parameter setter. This function bypasses the automatic
     * parameter generation to allow the developer to set values as if using
     * PDO directly.
     ...
     */
    final public function setValue(int|string $key, $value): self
    {
        if ($this->isUsingPlaceholders()) {
            $key = General::intval($key);
            if ($key === -1) {
                throw new DatabaseStatementException(
                    'Cannot use string index when using placeholders. Please use a numeric index.'
                );
            }
        ...
        if (isset($this->values[$key])) {
            throw new DatabaseStatementException(
                "Value for parameter `$key` is already defined"
            );
        }
     ...
     *  The current instance
     */
    final public function usePlaceholders(): self
    {
     ...
     */
    final public function isUsingPlaceholders(): bool
    {
     ...
     *  The current instance
     */
    final public function unsafe(): self
    {
        $this->safe = false;
        return $this;
    }
     ...
     * @return string
     */
    final public function computeHash(): string
    {
     ...
     */
    final public function execute(): DatabaseStatementResult
    {
        return $this
            ->finalize()
            ->getDB()
     ...
     * @return string
     */
    final public function replaceTablePrefix(string $table): string
    {
     ...
     */
    final public function asPlaceholderString(string $key, $value): string
    {
        if (!$this->isUsingPlaceholders() && Scalar::intval($key) === -1) {
     ...
     * @return string
     */
    final public function asPlaceholdersList(array $values): string
    {
     ...
     */
    final public function tickString(string $value): string
    {
     ...
     *  The arguments array
     */
    final public function splitFunctionArguments(string $arguments): array
    {
     ...
     * @throws Exception
     */
    final public function asTickedString(string|array $value, string $alias = null): string
    {
        if (!$value) {
            return '';
        }
     ...
     */
    final public function asTickedList(array $values): string
    {
     ...
     * @throws Exception
     */
    final protected function validateFieldName(string $field): void
    {
     ...
     * @throws Exception
     */
    final protected function validateTickedString(string $value): void
    {
        if ($value === '*') {
            return;
        }
     ...
     */
    final public function convertToParameterName(string $field, $value): string
    {
