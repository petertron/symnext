...
trait DatabaseWhereDefinition
{
     ...
     */
    final public function buildSingleWhereClauseFromArray(
        string $k,
        $c
    ): string
    {
        ...
        if (!is_string($k)) {
            throw new DatabaseStatementException(
                'Cannot use a number as a column name'
            );
        }
        // When we get here:
     ...
     * @return string
     */
    final public function buildWhereClauseFromArray(array $conditions): string
    {
        if (empty($conditions)) {
            throw new DatabaseStatementException(
                'Can not build where clause with an empty array'
            );
        }
        return implode(
            self::STATEMENTS_DELIMITER,
            General::array_map(
                [$this, 'buildSingleWhereClauseFromArray'], $conditions
            )
        );
    }
}
