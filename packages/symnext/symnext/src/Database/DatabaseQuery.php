...
use Symnext\Database\DatabaseQueryResult;
...
class DatabaseQuery extends DatabaseStatement
{
     ...
     *  The columns names to include in the projection.
     ...
     * @return void
     */
    protected function appendCacheModifier(): void
    {
     ...
     */
    final public function asProjectionList(array $values): string
    {
     ...
     *  The current instance
     */
    public function projection(array $projection = []): self
    {
     ...
     * @return DatabaseQuery
     */
    public function count(string $col = null): self
    {
        if (!$col) {
     ...
     */
    public function join(string $table, string $alias = null): DatabaseQueryJoin
    {
        $table = $this->replaceTablePrefix($table);
        $table = $this->asTickedString($table);
     ...
     *  An optional alias for the table. Defaults to null, i.e. no alias.
     * @return DatabaseQueryJoin
     *  A new instance of DatabaseQueryJoin linked to the current DatabaseQuery
     *  instance.
     */
     ...
     *  The current instance
     */
    public function orderBy(string|int|array $cols, string $direction = 'ASC'): self
    {
        $orders = [];
        $randoms = ['RAND()', 'RANDOM()'];
        if (!is_array($cols)) {
        ...
        foreach ($cols as $col => $dir) {
            ...
            if (!is_string($col)) {
                throw new Exception('Table column name must be string.');
            }
            if (!is_string($dir)) {
                throw new Exception('Order direction must be string.');
            }
        ...
        $op = $this->containsSQLParts('order by')
            ? self::VALUES_DELIMITER : 'ORDER BY';
    ...
    public function groupBy(string|array $columns): self
    {
        ...
        $op = $this->containsSQLParts('group by')
            ? self::VALUES_DELIMITER : 'GROUP BY';
     ...
     *  The current instance
     */
    public function paginate(int $page, int $size): self
    {
     ...
     *  The current instance
     */
    public function finalize(): DatabaseStatement
    {
     ...
     */
    public function select(array $values = []): DatabaseSubQuery
    {
     ...
     * @return DatabaseQuery
     */
    public function countProjection(string $col = '*'): self
    {
