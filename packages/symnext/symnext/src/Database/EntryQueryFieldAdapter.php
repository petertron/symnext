...
class EntryQueryFieldAdapter
{
     ...
     */
    public function formatColumn(string $col, int $field_id): string
    {
     ...
     */
    public function isFilterRegex(string $string): bool
    {
     ...
     */
    public function createFilterRegexp(string $filter, array $columns): ?array
    {
     ...
     */
    public function isFilterSQL(string $string): bool
    {
     ...
     * @throws DatabaseStatementException
     */
    public function createFilterSQL(string $filter, array $columns): array
    {
        $field_id = General::intval($this->field->get('id'));
        $filter = $this->field->cleanValue($filter);
        $op = '';
     ...
     *  The filter array
     */
    public function createFilterEquality(string $filter, array $columns): array
    {
        $field_id = General::intval($this->field->get('id'));
        $filter = $this->field->cleanValue($filter);
        $op = '=';
     ...
     *  The filter array
     */
    public function createFilterEqualityOrNull(string $filter, array $columns): array
    {
        $field_id = General::intval($this->field->get('id'));
        $filter = $this->field->cleanValue($filter);
        $op = '=';
     ...
     */
    public function isFilterNotEqual(string $string): bool
    {
     ...
     *  The filter array
     */
    public function createFilterNotEqual(string $filter, array $columns): array
    {
        $field_id = General::intval($this->field->get('id'));
    ...
    public function filter(
        EntryQuery $query, array $filters, string $operator = 'or'
    ): void
    {
        if (empty($filters)) {
            return;
     ...
     */
    public function sort(EntryQuery $query, ?string $direction = 'ASC')
    {
        $field_id = General::intval($this->field->get('id'));
